...
app.post("/api/assignments", ensureAdmin, async (req, res) => {
  const { ideaId, judgeIds } = req.body || {};
  const numericIdeaId = Number(ideaId);
  if (!Number.isFinite(numericIdeaId)) {
  ...
  try {
    const idea = await db.get(
      `SELECT id, idea_title, file_path FROM ideas WHERE id = ?`,
      [numericIdeaId]
    );
    if (!idea) {
    ...
    if (idea.file_path) {
      try {
        parsedFiles = JSON.parse(idea.file_path);
      } catch (err) {
        console.warn("Failed to parse idea.file_path for assignment", idea.id, err);
      }
    }

    const rawPdfPath =
      typeof parsedFiles?.pdf === "string" && parsedFiles.pdf.trim()
        ? parsedFiles.pdf.trim()
        : null;
    const normalizedPdfPath = normalizeUploadPath(rawPdfPath) ?? rawPdfPath;
    if (!normalizedPdfPath) {
      return res
        .status(400)
        .json({ error: "Idea does not have an associated PDF to assign" });
    }
    const pdfStoragePath = rawPdfPath ?? normalizedPdfPath;

    const uniqueJudgeIds = Array.from(
      new Set(
        judgeIds
          .map((id) => Number(id))
    ...
    if (uniqueJudgeIds.length === 0) {
      return res.status(400).json({ error: "No valid judge ids provided" });
    }
    ...
    for (const judgeId of uniqueJudgeIds) {
      const judge = await db.get(`SELECT id, name, username FROM judges WHERE id = ?`, [
        judgeId,
      ]);
      if (!judge) {
        skipped.push({ judgeId, reason: "Judge not found" });
        continue;
      }

      const existing = await db.get(
        `SELECT id FROM judge_projects WHERE judge_id = ? AND idea_id = ?`,
        [judgeId, numericIdeaId]
      );
      if (existing) {
        skipped.push({ judgeId, reason: "Already assigned" });
        continue;
      }

      const result = await db.run(
        `INSERT INTO judge_projects (judge_id, idea_id, description, pdf_path, status, final_score, evaluation, decision_at)
         VALUES (?, ?, ?, ?, 'PENDING', NULL, NULL, NULL)`,
        [
          judgeId,
          numericIdeaId,
          idea.idea_title ? String(idea.idea_title) : `Idea #${idea.id}`,
          pdfStoragePath,
        ]
      );
      const created = await db.get(`SELECT * FROM judge_projects WHERE id = ?`, [
        result.lastID,
      ]);
      inserted.push({
        id: created.id,
        judgeId: created.judge_id,
        ideaId: created.idea_id,
        status: created.status,
        pdf_url: toPublicPath(created.pdf_path),
      });
    }

    res.json({ ok: true, inserted, skipped });
  } catch (e) {
    console.error("create assignments error", e);
...
app.get("/api/admin/ideas/ranking", ensureAdmin, async (req, res) => {
  try {
    const rows = await db.all(`
      SELECT
        i.id AS idea_id,
      ...
      FROM ideas i
      LEFT JOIN judge_projects p ON p.idea_id = i.id
      LEFT JOIN judges j ON j.id = p.judge_id
      ORDER BY i.submitted_at DESC, p.id ASC
    `);
    ...
    for (const row of rows) {
      ...
      const entry = grouped.get(ideaId);
      if (row.assignment_id != null) {
        entry.judges.push({
          assignmentId: String(row.assignment_id),
          judgeId: row.judge_id != null ? String(row.judge_id) : null,
          judgeName: row.judge_name || null,
          judgeUsername: row.judge_username || null,
          status: row.assignment_status || "PENDING",
          score:
            row.final_score != null && Number.isFinite(Number(row.final_score))
    ...
    const result = Array.from(grouped.values()).map((entry) => {
      const scores = entry.judges
        .map((j) => (typeof j.score === "number" ? j.score : null))
        .filter((v) => v !== null);
      const averageScore =
        scores.length > 0
          ? Number((scores.reduce((sum, val) => sum + val, 0) / scores.length).toFixed(2))
          : null;
      const completedCount = entry.judges.filter((j) => typeof j.score === "number").length;
      const totalAssignments = entry.judges.length;
      ...
      return {
        ...
        completedCount,
        totalAssignments,
        latestActivity: latestDecision || entry.submittedAt || null,
      };
    });
    ...
    console.error("admin ideas ranking error", e);
    res.status(500).json({ error: "Failed to load ranking" });
  }
});

app.get("/api/admin/dashboard/summary", ensureAdmin, async (req, res) => {
  try {
    const totalsRow = await db.get(`SELECT COUNT(*) as totalIdeas FROM ideas`);
    const assignedRow = await db.get(
      `SELECT COUNT(DISTINCT idea_id) as assignedIdeas FROM judge_projects WHERE idea_id IS NOT NULL`
    );
    const completedRow = await db.get(
      `SELECT COUNT(*) as completed FROM judge_projects WHERE final_score IS NOT NULL`
    );

    const totalIdeas = totalsRow?.totalIdeas ?? 0;
    const assignedIdeas = assignedRow?.assignedIdeas ?? 0;
    const unassignedIdeas = Math.max(totalIdeas - assignedIdeas, 0);
    const completedEvaluations = completedRow?.completed ?? 0;

    const recentIdeas = await db.all(
      `SELECT id, idea_title, submitted_at FROM ideas ORDER BY submitted_at DESC LIMIT 5`
    );
    ...
    const recentEvaluations = await db.all(
      ...
      WHERE p.decision_at IS NOT NULL
      ORDER BY p.decision_at DESC
      LIMIT 5
    `
    );
    ...
    const events = [
      ...recentIdeas.map((i) => ({
        id: `idea-${i.id}`,
        type: "idea_submitted",
        timestamp: i.submitted_at || null,
        ideaId: String(i.id),
        ideaTitle: i.idea_title || `Idea #${i.id}`,
      })),
      ...recentEvaluations.map((ev) => ({
        id: `eval-${ev.id}`,
        type: "evaluation_completed",
        timestamp: ev.decision_at || null,
        ideaId: ev.idea_id != null ? String(ev.idea_id) : null,
        ideaTitle: ev.idea_title || (ev.idea_id != null ? `Idea #${ev.idea_id}` : null),
        judgeName: ev.judge_name || null,
        judgeUsername: ev.judge_username || null,
        score:
          ev.final_score != null && Number.isFinite(Number(ev.final_score))
            ? Number(ev.final_score)
            : null,
      })),
    ]
